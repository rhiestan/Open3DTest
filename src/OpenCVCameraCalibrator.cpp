...
OpenCVCameraCalibrator::OpenCVCameraCalibrator()
{
}
...
void OpenCVCameraCalibrator::setup()
{
	imagePoints_.clear();
}

void OpenCVCameraCalibrator::addNewImage(const open3d::geometry::Image& colorImg, const open3d::geometry::Image& depthImg)
{
	...
	unsigned char* dataPtr = colorImg.PointerAt<unsigned char>(0, 0, 0);
	memcpy(cvPtr, &(colorImg.data_[0]), 3 * colorImg.height_ * colorImg.width_);
	cv::cvtColor(colorI, colorO, cv::COLOR_RGB2BGR);

	imageSize_ = colorO.size();  // Format input image.

	cv::Size boardSize(7, 7);
	std::vector<cv::Point2f> pointBuf;
	int chessBoardFlags = cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_NORMALIZE_IMAGE | cv::CALIB_CB_FAST_CHECK;
	bool found = cv::findChessboardCorners(colorO, boardSize, pointBuf, chessBoardFlags);
	if (found)
	{
		cv::Mat viewGray;
		int winSize = 11;
		cv::cvtColor(colorO, viewGray, cv::COLOR_BGR2GRAY);
	...
	cv::drawChessboardCorners(colorO, boardSize, cv::Mat(pointBuf), found);

	cv::imwrite("color.png", colorO);
...
void OpenCVCameraCalibrator::reset()
{
	std::unique_lock<std::mutex> lock(mutex_);
	imagePoints_.clear();
...
void OpenCVCameraCalibrator::saveVolume()
{
	Settings s;
	...
	float grid_width = s.squareSize * (s.boardSize.width - 1);
	bool release_object = false;
	bool isOk = runCalibrationAndSave(s, imageSize_, cameraMatrix, distCoeffs, imagePoints_, grid_width,
		release_object);
